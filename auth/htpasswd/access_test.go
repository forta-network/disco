// Taken from github.com/distribution/distribution (registry/auth/htpasswd)
package htpasswd
...
func TestAllowReads(t *testing.T) {
	tempFile, err := ioutil.TempFile("", "htpasswd-test")
	if err != nil {
		t.Fatal("could not create temporary htpasswd file")
	}
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		ctx := context.WithRequest(ctx, r)
		_, err := accessController.Authorized(ctx)
		if err != nil {
			switch err := err.(type) {
			case auth.Challenge:
				err.SetHeaders(r, w)
	...
	for _, httpMethod := range []string{"GET", "HEAD", "OPTIONS"} {
		req, _ := http.NewRequest("GET", server.URL, nil)
		resp, err := client.Do(req)

		if err != nil {
			t.Fatalf("unexpected error during %s: %v", httpMethod, err)
		}
		defer resp.Body.Close()

		// Read request should be allowed
		if resp.StatusCode >= 300 {
			t.Fatalf("unexpected read response status: %v != %v", resp.StatusCode, http.StatusUnauthorized)
		}
	}
	...
	if err != nil {
		t.Fatalf("unexpected error during POST: %v", err)
	}
	defer resp.Body.Close()

	// Write request should not be allowed
	if resp.StatusCode != http.StatusUnauthorized {
		t.Fatalf("unexpected write response status: %v != %v", resp.StatusCode, http.StatusUnauthorized)
	}
}
