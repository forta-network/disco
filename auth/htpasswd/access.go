...
func newAccessController(options map[string]interface{}) (auth.AccessController, error) {
	allowReadsV, present := options["allowreads"]
	allowReads, ok := allowReadsV.(bool)
	if present && !ok {
		return nil, fmt.Errorf(`"allowreads" must be a boolean`)
	}
	...
	return &accessController{allowReads: allowReads, realm: realm.(string), path: path}, nil
}
...
func (ac *accessController) Authorized(ctx context.Context, accessRecords ...auth.Access) (context.Context, error) {
	...
	// Extend the original access controller: Let these requests reach when reads are allowed
	if ac.allowReads && (req.Method == "GET" || req.Method == "HEAD" || req.Method == "OPTIONS") {
		return ctx, nil
	}
